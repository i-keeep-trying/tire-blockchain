...
async function main() {
  ...
  const eventType = "SHIPPED_TO_RETAILER";

  // Simulated dispatch note
  const buf = new TextEncoder().encode("dst_to_rtl_dispatch_note.pdf: demo");
  const offchainHash = ethers.keccak256(buf);
  const offchainURI = "ipfs://dst_to_rtl_dispatch_note.pdf";

  const tx = await contract.recordEvent(tireId, eventType, offchainHash, offchainURI);
  console.log("Ship→Retailer tx:", tx.hash);
